...
theN <- 3565000
make_params <- function(Beta = 0.421,
                        N = theN,
                        gamma = 0.0876,
                        delta = 0.0028,
                        ##Awarness constant.
...
make_age_inits <- function(I0= c(1, 1, 1, 1, 1), N = theN){
  #######I MADE ONE CHANGE HERE -JASON
  #######Initial susceptables in age group highly variable as it 
  #######DEPENDS HEAVILY ON POPULATION STUCTURE. Dug up CN census data
  #######AND GOT THE PROPORTION OF POP IN EACH AGE GROUP. ADDED HERE
  ##Cite Dr. Bolker's Feedback on Milestone 1 
  n_age <- length(I0)
  struct = c(0.105587012,0.257629189,0.245480097,0.269248731,0.122054971)
 state <- unlist(c(S=(N-I0)*struct, I=I0, Q=rep(0,n_age), R=rep(0, n_age),
D=rep(0,n_age)))
 return(state)
...
This next few code chunks provide a nice framework for plotting the CN covid data by town, with plenty of options. meant for checking spacial heterogeniety condition. 
```{python}
...
#These are graph settings for Seaborn. Can ignore if trying to understand code --- they mask matplotlib features I'm pretty sure.
...
library(reticulate)
#LOESS SMOOTHING
#Actually really really bad. What a shame.
...
plt.legend()
plt.title("DEATH Count by Age Group (rolling average)")
...
errorDeaths =  function(parameters){
  ...
  sim_results = run_sim(params = parameters, length = (length(py$Deaths10_29$Deaths) - 1 ))
  ...
  return(  MSE(s1, py$Deaths0_9$Deaths) + 
...
errorCases = function(parameters){
  ...
  sim_results = run_sim(params = parameters, length = (length(py$df10_29$Cases) -1))
...
errorDeaths(initparams)
results = optim(par = initparams, fn = errorDeaths, method = "L-BFGS-B", control = list(maxit = 1))

results
```

```{r}
results$par == make_params()
...
sim_results = run_sim(params = para, length = (length(py$df10_29$Cases) ))
TotalCases = diff(sim_results$I2 + sim_results$Q2 + sim_results$R2 +sim_results$D2)
TotalInfected2 = sim_results$I2


s1 = diff(cumsum(sim_results$I1))
#plot(NewCases2)
plot(sim_results$I4)
#plot(sim_results$R1)
#plot(py$df10_29$Cases)

#plot(cumsum(-NewCases2))
#plot(cumsum(py$df10_29$Cases))
#plot(sim_results$I1)
```
